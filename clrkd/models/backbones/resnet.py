...
from clrkd.models.registry import BACKBONES
...
def calculate_attention_map(x):
    # Compute the attention map as the normalized mean of the squared outputs across the channel dimension
    attention_map = x.pow(2).mean(dim=1, keepdim=True)
    normalized_attention_map = F.normalize(attention_map, p=2, dim=[2, 3])
...
class ResNetWrapper(nn.Module):
    ...
    def forward(self, x, att_flag=False):
        if att_flag:
            x, attention_maps = self.model(x, att_flag)
...
class ResNet(nn.Module):
    ...
    def forward(self, x, att_flag = False):
        ...
        for name in ['layer1', 'layer2', 'layer3', 'layer4']:
            ...
            attention_map = calculate_attention_map(x)
            attention_maps.append(attention_map)
        if att_flag:
            return out_layers, attention_maps
        else:
            return out_layers
