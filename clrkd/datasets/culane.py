...
import clrkd.utils.culane_metric as culane_metric
import cv2
...
class CULane(BaseDataset):
    def __init__(self, data_root, split, processes=None, cfg=None):
        ...
        self.split = split
        self.diff_path = cfg['diff_path']
        self.threshold = cfg.threshold
        self.load_diffs()  # Load the frame-to-frame differences
        self.load_annotations()

        # Print the number of images loaded
        print(f'Number of images loaded: {len(self.data_infos)}')
        self.logger.info(f'Number of images loaded: {len(self.data_infos)}')

    def load_diffs(self):
        if self.diff_path:
            self.logger.info(f'Loading frame-to-frame differences from {self.diff_path}...')
            npz_file = np.load(self.diff_path)
            self.frame_diffs = npz_file['data']
        else:
            self.logger.info('No frame-to-frame difference file provided.')
    ...
    def load_annotations(self):
        ...
        if os.path.exists(cache_path):
            with open(cache_path, 'rb') as cache_file:
                # Load all data first
                all_data_infos = pkl.load(cache_file)
            
            # Filter data based on frame differences if applicable
            self.data_infos = []
            if self.frame_diffs is not None and self.training:
                for idx, data in enumerate(all_data_infos):
                    if idx < len(self.frame_diffs) and self.frame_diffs[idx] > self.threshold:
                        self.data_infos.append(data)
                self.max_lanes = max(len(anno['lanes']) for anno in self.data_infos) if self.data_infos else 0
            else:
                self.data_infos = all_data_infos
                self.max_lanes = max(len(anno['lanes']) for anno in self.data_infos) if self.data_infos else 0
            return
